...
def hourly_avg(df): #df has index datetime

    # average wind_dir weighted by speed
    temp = df[['cos_wind_dir','sin_wind_dir','speed']]
    s1 = (temp['cos_wind_dir'] * temp['speed']).resample('H', label='right').sum() / (temp['speed'].resample('H', label='right')).sum()
    s2 = (temp['sin_wind_dir'] * temp['speed']).resample('H', label='right').sum() / (temp['speed'].resample('H', label='right')).sum()
...
#
#     #
#     s1 = df['speed'].resample('D', label='right').min()
#     s2 = df['speed'].resample('D', label='right').max()
#     s3 = df['temp'].resample('D', label='right').min()
#     s4 = df['temp'].resample('D', label='right').max()
#     s5 = df['radiation'].resample('D', label='right').min()
#     s6 = df['radiation'].resample('D', label='right').max()
#
